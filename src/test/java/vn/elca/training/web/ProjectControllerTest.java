...
class ProjectControllerTest {
  ...
  class FindAllProjects {
    ...
    @Test
    @DisplayName("Should return 200 with page response when projects exist")
    void findAllProjects_shouldReturn200_whenProjectsExist() throws Exception {
      Page<ProjectDto> projectPage = new PageImpl<>(
          Collections.singletonList(projectDto),
          PageRequest.of(0, 5, Sort.by("projectNumber").ascending()),
          1
      );
      ...
      mockMvc.perform(get(PROJECTS_API).accept(MediaType.APPLICATION_JSON))
          ...
          .andExpect(jsonPath("$.data[0].projectNumber").value(3116));

      verify(projectService).findAll(any(Pageable.class));
    }

    @Test
    @DisplayName("Should return 200 with empty data when no projects exist")
    void findAllProjects_shouldReturnEmptyPage_whenNoProjects() throws Exception {
      Page<ProjectDto> emptyPage = new PageImpl<>(
          Collections.emptyList(),
          PageRequest.of(0, 5, Sort.by("projectNumber").ascending()),
          0
      );
      when(projectService.findAll(any(Pageable.class))).thenReturn(emptyPage);
  ...
  class SearchProjects {
    ...
    void search_shouldReturn200_whenCriteriaProvided() throws Exception {
      ...
      when(projectService.findProjectsByCriteria(eq("fact"), eq(Status.NEW), any(Pageable.class)))
      ...
      verify(projectService).findProjectsByCriteria(eq("fact"), eq(Status.NEW), pageableCaptor.capture());
    ...
    void search_shouldUseCustomPageAndSize_whenProvided() throws Exception {
      when(projectService.findProjectsByCriteria(isNull(), isNull(), any(Pageable.class)))
      ...
      ArgumentCaptor<Pageable> pageableCaptor = ArgumentCaptor.forClass(Pageable.class);
      verify(projectService).findProjectsByCriteria(isNull(), isNull(), pageableCaptor.capture());
    ...
    void search_shouldReturn400_whenKeywordTooLong() throws Exception {
      ...
      verify(projectService, never()).findProjectsByCriteria(anyString(), any(), any(Pageable.class));
    }
  }
  ...
  class UpdateProject {
    ...
    @Test
    @DisplayName("Should return 200 with updated project when request is valid")
    void updateProject_shouldReturn200_whenRequestIsValid() throws Exception {
      when(projectService.updateProject(eq(100L), any(ProjectUpdateRequest.class))).thenReturn(projectDto);
